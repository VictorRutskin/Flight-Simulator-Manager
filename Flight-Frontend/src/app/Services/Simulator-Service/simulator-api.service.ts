...
export class SimulatorApiService {
  ...
  public RandomAction(): Observable<string> {
    return this.http.post<string>(environment.apiUrl + this.FlightsAPI+"/RandomAction", null);
  }
  ...
  public updateFlight(flightNumber: string, flight: any): Observable<any> {
    return this.http.put<any>(environment.apiUrl + this.FlightsAPI + `/${flightNumber}`, flight);
  }

  // Get Flights by Type
  public getFlightsByType(type: string): Observable<any[]> {
    return this.http.get<any[]>(environment.apiUrl + this.FlightsAPI + `/${type}`);
  }
